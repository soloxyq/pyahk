...
| 等待超过 ≈500ms 的动作被丢弃(而不是迟发) | **特性**:年龄绑在每个队列项上(入队记录 `QueueTickCount`),**出队时**判定:真实年龄 ≥ `STALE_TICKS`(32 tick)且可丢 → 丢弃并上报。过期决策没有价值,执行一个 500ms 前的决策比不执行更糟。不可丢动作(release/cleanup/seqrun)再老也照常执行 |
