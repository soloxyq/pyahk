...
```json
{
  "priority_keys": {
    ...
    "delay_ms": 50,
    "special_keys": ["space"],
    "description": "混合控制：space保持游戏原生响应，E和right_mouse由程序接管"
  }
}
```
...
#### 配置说明
- **keys**: 所有优先级按键列表
- **delay_ms**: 前置延迟时间（毫秒），确保游戏状态同步
- **special_keys**: 保持游戏原生响应的按键（不拦截系统事件）
- **managed_keys**: 自动计算（keys中除special_keys外的按键），程序完全接管
