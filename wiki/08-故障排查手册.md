                ...
                if key_part.startswith('delay'):
                    try:
                        ...
                        if delay_ms < 0 or delay_ms > 10000:
                            issues.append(f"延迟时间超出范围: {key_part}")
                    except ValueError:
                        issues.append(f"延迟格式错误: {key_part}")
        ...
        for key_part in alt_key.split(','):
            ...
            if key_part.startswith('delay'):
                try:
                    delay_ms = int(key_part[5:])
                    if delay_ms < 0 or delay_ms > 10000:
                        issues.append(f"辅助按键延迟时间超出范围: {key_part}")
                except ValueError:
                    issues.append(f"辅助按键延迟格式错误: {key_part}")
...
1. **检查序列格式**
   ...
   # 错误示例
   "key": "delay 50,q"      # 空格错误
   "key": "delay50, q"      # 多余空格（会被trim掉，但不推荐）
   "key": "delay50;q"       # 分隔符错误
   "key": "delay=50,q"      # 等号错误（正确格式无等号）
   
   # 正确示例
   "key": "delay50,q"       # 标准格式
   "key": "q,delay100,w"    # 多元素序列
   ```
...
3. **检查队列状态**
   ```python
   # 查看输入处理器日志
   LOG_INFO(f"[队列处理器] 处理按键序列: {key_sequence}")
   LOG_INFO(f"[队列状态] 当前队列长度: {queue_length}")
   ```
