...
优先级按键系统基于**Windows Hook 低级监听**实现了**选择性系统事件拦截**，允许部分按键（如空格键）保持游戏原生响应，而其他按键（如 E 键、右键）由程序完全接管。这种混合模式在提供程序精确控制的同时，确保关键操作（如闪避）的游戏体验不受影响。

**🔥 重大升级 (2025.01)**: 从 pynput 迁移到 CtypesHotkeyManager，彻底解决了输入无响应问题。

### 核心架构：Windows Hook 选择性事件拦截
...
| ------------ | ---------- | ------------ | ------ | ------- | -------------------------- |
| **矩形检测** | -          | HSV 颜色匹配 | ~5ms   | 90%+    | 传统条形资源条             |
| **圆形检测** | -          | HSV 颜色匹配 | ~5ms   | 95%+    | 球形/圆形资源条            |
| **Text OCR** | 模板匹配   | 模板匹配     | ~7ms   | 90-95%  | **推荐**，最快，无额外依赖 |
| **Text OCR** | Keras 模型 | 深度学习     | ~47ms  | >99%    | 最高准确率                 |
| **Text OCR** | Tesseract  | OCR 引擎     | ~240ms | 95-100% | 通用性强，无需训练         |

### 一、矩形检测模式

#### 工作原理

基于 HSV 颜色空间匹配，检测矩形区域内的资源条颜色变化。适用于传统的条形 HP/MP 显示。
...
| **Keras 模型** | ~47ms  | >99%    | TensorFlow | ✅ 是    | 追求最高准确率                 |
...
| 场景             | 推荐模式             | 理由                 |
| ---------------- | -------------------- | -------------------- |
| 数字显示的 HP/MP | Text OCR (模板匹配)  | 最快速度，无额外依赖 |
