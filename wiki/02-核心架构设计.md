...
def _publish_status_update(
    self,
    stationary_mode: Optional[bool] = None,  # ❌ 参数被忽略
    force_move_active: Optional[bool] = None,  # ❌ 参数被忽略
):
    # 方法内部总是使用内部状态，参数完全被忽略
...
def __init__(self, ...):
    # ...
    self._priority_keys_config = {}  # 初始为空
    self._setup_event_subscriptions()
    self._start_priority_listeners()  # ❌ 此时配置为空，无意义调用
```
...
def __init__(self, ...):
    # ...
    self._priority_keys_config = {}
    self._setup_event_subscriptions()
    # 注意：优先级按键监听将在配置加载后通过 _on_config_updated 启动
```

**改进效果**:
- ✅ 消除无效调用：避免在配置为空时尝试注册监听器
- ✅ 逻辑清晰：监听器在配置加载后自动启动
- ✅ 代码简洁：移除冗余代码

### 架构设计原则

通过持续的代码审查和重构，确保以下设计原则：

1. **API一致性**: 方法签名与实现行为严格一致
2. **初始化顺序**: 组件初始化遵循依赖关系
3. **状态完整性**: 状态发布总是包含完整信息
4. **代码简洁性**: 移除冗余和无效代码
5. **可维护性**: 清晰的注释和文档
