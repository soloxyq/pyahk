...
### 2025.10：AHK输入系统重构
- **完全重写**：采用AHK v2 + WM_COPYDATA架构，替代Python Hook
- **异步delay**：DelayUntil状态机，完全非阻塞
- **零硬编码**：所有按键配置由Python传入，AHK无硬编码
- **简化架构**：移除Python层暂停，单一暂停机制（AHK的DelayUntil）
- **统一队列**：所有按键操作都走队列系统，逻辑清晰
...
1. **异步delay**：DelayUntil状态机，完全非阻塞
2. **单层暂停**：只在AHK层暂停，Python层不参与
3. **零硬编码**：所有按键配置由Python传入
4. **统一队列**：所有按键操作都走队列系统
5. **状态去重**：monitor按键避免重复发送事件
6. **自动恢复**：delay结束后自动恢复，无需手动管理
