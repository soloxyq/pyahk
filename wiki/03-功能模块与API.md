...
### 概述
智能药剂系统提供自动化的HP/MP检测和药剂使用功能。支持**三大类检测方式**，每类都有其独特的优势和适用场景。

### 三大类检测方式总览

| 类别 | 子类型 | 技术 | 性能 | 准确率 | 适用场景 |
|------|--------|------|------|--------|----------|
| **矩形检测** | - | HSV颜色匹配 | ~5ms | 90%+ | 传统条形资源条 |
| **圆形检测** | - | HSV颜色匹配 | ~5ms | 95%+ | 球形/圆形资源条 |
| **Text OCR** | 模板匹配 | 模板匹配 | ~7ms | 90-95% | **推荐**，最快，无额外依赖 |
| **Text OCR** | Keras模型 | 深度学习 | ~47ms | >99% | 最高准确率 |
| **Text OCR** | Tesseract | OCR引擎 | ~240ms | 95-100% | 通用性强，无需训练 |

### 一、矩形检测模式

#### 工作原理
基于HSV颜色空间匹配，检测矩形区域内的资源条颜色变化。适用于传统的条形HP/MP显示。
...
#### 工作原理
基于HSV颜色空间匹配，检测圆形区域内的资源条颜色变化。适用于球形或圆形的HP/MP显示。
...
#### 工作原理
直接识别游戏中显示的HP/MP数字文本（如"540/540"），无需依赖颜色变化。支持三种OCR引擎，满足不同需求。

#### 三种OCR引擎对比

| 引擎 | 速度 | 准确率 | 依赖 | 需要训练 | 推荐场景 |
|------|------|--------|------|----------|----------|
| **模板匹配** | ~7ms | 90-95% | 无 | ✅ 是 | **默认推荐**，最快，无额外依赖 |
| **Keras模型** | ~47ms | >99% | TensorFlow | ✅ 是 | 追求最高准确率 |
| **Tesseract** | ~240ms | 95-100% | Tesseract | ❌ 否 | 无需训练，通用性强 |
...
**技术特性**:
- **无额外依赖**: 不需要安装TensorFlow或Tesseract
- **极致性能**: 单个字符识别~1.3ms，完整HP/MP识别~7ms
...
- **深度学习**: 基于CNN神经网络，识别能力强
- **批量优化**: 支持批量识别，5个字符一次处理
...
| 场景 | 推荐模式 | 理由 |
|------|----------|------|
| 数字显示的HP/MP | Text OCR (模板匹配) | 最快速度，无额外依赖 |
| 追求极致准确率 | Text OCR (Keras) | >99%准确率 |
| 无法训练模板 | Text OCR (Tesseract) | 无需训练，开箱即用 |
| 传统条形资源条 | 矩形检测 | 性能最佳，配置简单 |
| 球形资源条 | 圆形检测 | 更精确的圆形区域检测 |
